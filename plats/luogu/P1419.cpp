...
bool check(double m){
    ...
    double ans = 0;
    for (int i = s; i <= n; i++){
        while (!q.empty() && pre[q.back()] > pre[i - s])
            q.pop_back(); // 最大单调队列，
        q.push_back(i-s);//i-s是右边的点
        while (!q.empty() && q.front() < i - t)
            q.pop_front(); // i-t是左边的点，如果当前的点front比i-t还小，那就不在范围里，应该舍弃
        if (!q.empty() && pre[i] - pre[q.front()] >= 0)
            return true; // 如果已经存在这样的序列那就直接返回了，然后枚举更大的平均值
    }
    return false;
}
